...
\begin{frame}{Diagrammatic approach (\DRbar\ codes)}
  ...
  \begin{table}
    ...
    \begin{tabular}{lllll}
      Input & & & & Output \\
      \midrule
      $\alpha_\text{em}^{\SM(5)}(M_Z)$ & $\rightarrow$ & $e^\MSSM(M_Z)$ & $\rightarrow$ & $g_1^\MSSM(M_Z)$ \\
...
\begin{frame}{Diagrammatic approach (\DRbar\ codes)}
  ...
  \emph{Disadvantage:} suffers from large logarithms $\log(M_Z/M_S)$\\
\end{frame}
...
\begin{frame}{EFT approach}
  ...
  \begin{table}
    ...
    \begin{tabular}{lllll}
      Input & & & & Output \\
      \midrule
      $\alpha_\text{em}^{\SM(5)}(M_Z)$ & $\rightarrow$ & $e^\SM(M_Z)$ & $\rightarrow$ & $g_1^\SM(M_Z)$ \\
...
\begin{frame}{Combining EFT and diagrammatic approach: matching $M_h$}
  ...
  \begin{align*}
    \lambda = \frac{1}{v^2} \left[ (M_h^\text{MSSM})^2 +
      \Sigma^\SM_h(p^2 = (M_h^\SM)^2,\mu = M_Z) \right]
  \end{align*}
  where
  \begin{align*}
    (M_h^{\SM})^2 &= \lambda v^2 - \Sigma^{\SM}_h(p^2 = (M_h^{\SM})^2,\mu = M_Z) \\
    (M_h^\text{MSSM})^2 &= \text{smallest eigenvalue of} \\
    &\phantom{={}} \left[(m_h^\text{MSSM})^2 - \Sigma^\text{MSSM}_h(p^2 = (M_h^\text{MSSM})^2,\mu = M_Z)\right]_{ij}
  \end{align*}
  \emph{Advantage:} resums large logarithms $\log(M_Z/M_S)$\\
...
% \begin{frame}{Comparison diagrammatic / EFT}
%   \begin{figure}
%     \centering
...
\begin{frame}{Summary}
  \emph{Summary:}
  \begin{itemize}
  \item diagrammatic approach suffers from large logs $\log(M_Z/M_S)$
  \item EFT approach misses terms $O(p^2/M_S^2)$
  \item mixed approach \emph{resums large logs and includes terms
      $O(p^2/M_S^2)$}
  \item mixed approach can be \emph{automatized} $\rightarrow$
