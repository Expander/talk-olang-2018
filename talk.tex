...
\begin{frame}{Automatic EFT approach}
  ...
  \end{center}
  \emph{where $\lambda(M_S)$ is determined by numerical 1L matching condition}
\end{frame}
...
\section{Equivalence of EFT and automatic EFT approach}

\begin{frame}{Equivalence EFT and automatic EFT approach $O(y_t^2)$}
  \begin{align*}
    M_h^{\SM} &\overset{!}{=} M_h^\text{MSSM} \quad \text{at} \quad Q = M_S, 1L
  \end{align*}
  where
