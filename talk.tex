...
\begin{frame}{Supersymmetry}
  ...
  \visible<1>{
    \tikz[overlay,remember picture]
    \node[anchor=north east] at ($(current page.north east)+(-1,-1.2)$) {
      \includegraphics[scale=0.7]{images/DM_SUSY}
    };
  }
  \visible<2>{
    \tikz[overlay,remember picture]
  ...
  \visible<3>{
    \tikz[overlay,remember picture]
    \node[anchor=north east] at ($(current page.north east)+(-0.8,-2.6)$) {
      \fbox{$\lambda = \frac{1}{4} \textcolor{darkgreen}{(g_Y^2 + g_2^2)} c_{2\beta}^2$}
    };
  }
  \visible<4>{
    \tikz[overlay,remember picture]
    \node[anchor=north east] at ($(current page.north east)+(-0.8,-2.6)$) {
      \fbox{$M_h^2 = \textcolor{darkgreen}{m_Z^2} c_{2\beta}^2$}
    };
  }
  \visible<5>{
    \tikz[overlay,remember picture]
    \node[anchor=north east] at ($(current page.north east)+(-0.8,-2.6)$) {
      \fbox{$\Lagr = \textcolor{darkgreen}{\Delta_{27,ij}} \phi\psi_i\psi_j$}
    };
  }
  \visible<6>{
    \tikz[overlay,remember picture]
    \node[anchor=north east] at ($(current page.north east)+(-0.5,-2.6)$) {
      \fbox{\scalebox{0.6}{$\fmfvcenter{htop} + \fmfvcenter{hstop} = 0$}}
    };
  }
  \visible<7>{
    \tikz[overlay,remember picture]
    \node[anchor=north east] at ($(current page.north east)+(-0.8,-2.6)$) {
      \fbox{$\textcolor{darkgreen}{E_8\times E_8} \rightarrow E_6 \rightarrow \textcolor{darkgreen}{G_\SM}$}
    };
  }
\end{frame}
