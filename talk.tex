...
\begin{frame}{Available models (selection)}
  \begin{table}
    \centering
    \begin{tabular}{llll}
      Models            & RGEs & \multicolumn{2}{l}{diagrammatic $M_h$ contributions} \\
      \midrule
      ...
      THDM + split      & 2L   & 1L \\
      \\
      EFTs              & RGEs & diagrammatic $M_h$ & matching \\
                        &      & contributions      & conditions \\
      \midrule
      split-MSSM        & 2L   & 1L + 2L $O(\alpha_t (\alpha_s + \alpha_t))$ \\
                        &      & \phantom{1L} + 3L gluino $O(\alpha_t \alpha_s^2)$ \\
      SM                & 3L   & 1L + 2L $O(\alpha_t (\alpha_s + \alpha_t))$ \\
      SM                & 3L   & 1L + 2L $O(\alpha_t (\alpha_s + \alpha_t))$ \\
    \end{tabular}
  \end{table}
\end{frame}
