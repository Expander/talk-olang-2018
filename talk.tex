...
\begin{frame}{Incorrect 2L logs in FlexibleEFTHiggs-1L}
  ...
  \end{align*}
  $\Rightarrow$\\
  incorrect 2L logs remain in FlexibleEFTHiggs
\end{frame}
...
\begin{frame}{Improvement 1: FlexibleEFTHiggs-2L}
  ...
  \item correctly resums LL
  \item all non-log terms correct at 1L, \\
    including all terms $O(p^n/M_S^n, v^n/M_S^n)$
  \item non-log terms correct at 2L
    $O(\alpha_s(\alpha_t + \alpha_b) + (\alpha_t + \alpha_b)^2 + \alpha_\tau^2)$
  \end{itemize}
  \emph{Disadvantage:}
  \begin{itemize}
  \item incorrect 3L logs $O(\hbar^3 \log^2(M_t/M_S))$
  \end{itemize}
\end{frame}
...
\begin{frame}{Improvement 2: FlexibleEFTHiggs-1L\fsstar}
  \begin{center}
    ...
    but avoiding appearance of incorrect higher-order logs \\
...
\begin{frame}{Improvement 2: FlexibleEFTHiggs-1L\fsstar}
  ...
  \begin{align*}
    ...
    (M_h^\MSSM)^2 &= M_{h}^{(1)} - \Sigma^{\MSSM}_h((m_h^\MSSM)^2) + \tilde{t}_h^\MSSM
  \end{align*}
  with
  \begin{align*}
...
\begin{frame}{Improvement 2: FlexibleEFTHiggs-1L\fsstar}
  \emph{Advantages:}
  \begin{itemize}
  \item ??? correctly resums LL + NLL
  \item ??? all non-log terms correct at 1L, \\
    including all terms $O(p^n/M_S^n, v^n/M_S^n)$
  \end{itemize}
  \emph{Disadvantage:}
  \begin{itemize}
  \item more difficult to add 2L corrections
  \item non-logarithmic 2L terms arise at $M_S$
  \end{itemize}
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{frame}{Summary FlexibleEFTHiggs}
  \emph{FlexibleEFTHiggs-1L:}\\
  Advantages:
  \begin{itemize}
  \item easily automatizable
  \item correctly resums LL
  \item all non-log terms correct at 1L, including all terms $O(p^n/M_S^n, v^n/M_S^n)$
  \end{itemize}
  Disadvantage:
  \begin{itemize}
  \item incorrect 2L logs $O(\hbar^2 \log(M_t/M_S))$
  \end{itemize}
