...
\begin{frame}{Incorrect 2L logs in FlexibleEFTHiggs-1L}
  ...
  \end{align*}
  $\Rightarrow$\\
  large 2L logs remain in FlexibleEFTHiggs-1L\\[1em]
  $\Rightarrow$\\
  FlexibleEFTHiggs-1L: large 2L logs $O(\hbar^2 \log(M_t/M_S))$
  FlexibleEFTHiggs-2L: large 3L logs $O(\hbar^3 \log^2(M_t/M_S))$
\end{frame}
...
\begin{frame}{EFT approach with $M_h$ matching (FlexibleEFTHiggs)}
  ...
  \item FlexibleEFTHiggs-1L: correctly resums LL
  \item FlexibleEFTHiggs-2L: correctly resums LL + NLL
...
\begin{frame}{Summary}
  ...
  \item pure EFT approach resums large logs but neglects terms
    $O(p^2/M_S^2)$.  Also difficult to automatize.
  \item FlexibleEFTHiggs
    \begin{itemize}
    \item includes all 1L terms $O(p^n/M_S^n, v^n/M_S^n)$
    \item FlexibleEFTHiggs-1L resums LL (but incorrect NLL)
    \item FlexibleEFTHiggs-2L resums LL + NLL (but incorrect NNLL)
