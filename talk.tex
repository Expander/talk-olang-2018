...
\begin{frame}{Diagrammatic approach (\DRbar\ codes)}
  \centering
  \includegraphics[width=0.45\textwidth]{images/mssm-sm-tower-diagrammatic}
\end{frame}

\begin{frame}{Diagrammatic approach (\DRbar\ codes)}
  \emph{Idea:} Calculate $M_h$ in the MSSM as a function of the \DRbar\ parameters:\\[1.5em]
...
\begin{frame}{Diagrammatic approach (\DRbar\ codes)}
  ...
  \begin{table}
    ...
    \begin{tabular}{lllll}
      ...
      $M_t$ & $\rightarrow$ & $m_Z^\MSSM(M_t)$ & $\rightarrow$ & $v^\MSSM(M_t)$ \\
  ...
  \emph{Free parameters:} $\mu$, $B\mu$, $m_{\tilde{f},ij}^2$, $M_i$,
  $T^f_{ij}$
\end{frame}

\begin{frame}{Diagrammatic approach}
  \begin{align*}
...
\begin{frame}{EFT approach: matching all $\Gamma_{\phi_1,\ldots,\phi_n}$}
  ...
  \begin{align*}
    ...
    &\phantom{={}}
    + \Delta \lambda^{1L,\,\text{reg}}
    + \Delta \lambda^{1L,\,\phi}
    + \Delta \lambda^{1L,\,\chi}
  \end{align*}
  \emph{Advantage:} resums large logarithms $\log(M_Z/M_S)$\\
  \emph{Disadvantage:} neglects 1L terms $O(p^2/M_S^2)$
\end{frame}
...
\begin{frame}{Combining EFT and diagrammatic approach}
  ...
  %   \includegraphics[width=\textwidth]{plots/PlotScale-in-FH_new_low-selected}
  % \end{figure}
  Bild: calculate $\lambda(M_S)$ including $p^2/M_S^2$ terms
\end{frame}
