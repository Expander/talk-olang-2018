...
\begin{frame}{Available full MSSM models and EFTs}
  \resizebox{\textwidth}{!}{%
    \begin{tabular}{llll}
      ...
      \midrule
      MSSM              & 3L   & 1L + 2L $O((\alpha_t + \alpha_b) \alpha_s)$ & --\\
      (``full model'')  &      & \phantom{1L} + 2L $O((\alpha_t + \alpha_b)^2)$ \\[0.5em]
      ...
      (``EFT'')         &      &                                            & + 2L $\lambda$ $O((\alpha_s + \alpha_t)\alpha_t^2)$ \\
                        &      &                                            & \mycite{1407.4081, 1504.05200} \\[0.5em]
      SM                & 3L   & 1L                                         & 1L $\lambda$ + $O(p^2/M_S^2)$ terms \\
      (``automatic EFT'')
    \end{tabular}}
\end{frame}
...
% \begin{frame}[noframenumbering]{Comparison full / EFT}
%   \begin{figure}
%     \centering
