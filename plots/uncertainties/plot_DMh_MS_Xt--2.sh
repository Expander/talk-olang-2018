...
outfile = directory + r'DMh_MS_TB-5_Xt--2.pdf'

try:
    dataMSSMEFTHiggs = np.genfromtxt(directory + r'MSSMEFTHiggs_MS_TB-5_Xt--2..dat')
    dataMSSM         = np.genfromtxt(directory + r'NUHMSSMNoFVHimalaya_MS_TB-5_Xt--2..dat')
    dataHSSUSY       = np.genfromtxt(directory + r'HSSUSY_MS_TB-5_Xt--2..dat')
except:
...
plt.title(r'$X_t = -2 M_S, \tan\beta = 5$', color='k', fontsize=10)
